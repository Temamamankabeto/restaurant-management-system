...
class CreditOrderController extends Controller
{
    ...
    public function accounts(Request $request)
    {
        ...
        if ($request->filled('search')) {
            ...
            $q->where(fn($x) => $x->where('name', 'like', "%{$s}%")->orWhere('phone', 'like', "%{$s}%")->orWhere('email', 'like', "%{$s}%"));
        }
        if ($request->filled('status')) $q->where('status', $request->status);
        if ($request->filled('account_type')) $q->where('account_type', $request->account_type);
        $rows = $q->paginate(max(1, min((int) $request->query('per_page', 10), 100)));
        return response()->json(['success' => true, 'data' => $rows->items(), 'meta' => ['current_page' => $rows->currentPage(), 'last_page' => $rows->lastPage(), 'per_page' => $rows->perPage(), 'total' => $rows->total()]]);
    }
    ...
    public function storeAccount(Request $request)
    {
        ...
        return response()->json(['success' => true, 'message' => 'Credit account created successfully.', 'data' => $account], 201);
    }
    ...
    public function updateAccount(Request $request, $id)
    {
        ...
        $account->update($data);
        return response()->json(['success' => true, 'message' => 'Credit account updated successfully.', 'data' => $account->fresh()]);
    }
    ...
    public function orders(Request $request)
    {
        ...
        if ($request->filled('search')) {
            $s = trim((string) $request->search);
            $q->where(fn($x) => $x->where('credit_reference', 'like', "%{$s}%")->orWhereHas('order', fn($o) => $o->where('order_number', 'like', "%{$s}%"))->orWhereHas('account', fn($a) => $a->where('name', 'like', "%{$s}%")));
        }
        $rows = $q->paginate(max(1, min((int) $request->query('per_page', 10), 100)));
        return response()->json(['success' => true, 'data' => $rows->items(), 'meta' => ['current_page' => $rows->currentPage(), 'last_page' => $rows->lastPage(), 'per_page' => $rows->perPage(), 'total' => $rows->total()]]);
    }
    ...
    {
        $this->requirePermission($request, 'credit.orders.read');
        return response()->json(['success' => true, 'data' => CreditOrder::with(['account','order.items.menuItem','bill','settlements.receiver','logs.actor'])->findOrFail($id)]);
    }
    ...
    public function createFromBill(Request $request, $billId)
    {
        ...
        if (!empty($data['override_limit'])) $this->requirePermission($request, 'credit.orders.override');
        $creditOrder = $this->creditOrderService->createForBill(Bill::findOrFail($billId), (int) $data['credit_account_id'], (int) $request->user()->id, $data['due_date'] ?? null, $data['notes'] ?? null, (bool)($data['override_limit'] ?? false));
    ...
    public function approve(Request $request, $id)
    {
        ...
        return response()->json(['success' => true, 'message' => 'Credit order approved.', 'data' => $this->creditOrderService->approve(CreditOrder::findOrFail($id), (int) $request->user()->id, $data['note'] ?? null)]);
    }
    ...
    public function reject(Request $request, $id)
    {
        $this->requirePermission($request, 'credit.orders.approve');
        $data = $request->validate(['note' => 'nullable|string|max:1000']);
        return response()->json(['success' => true, 'message' => 'Credit order blocked/rejected.', 'data' => $this->creditOrderService->reject(CreditOrder::findOrFail($id), (int) $request->user()->id, $data['note'] ?? null)]);
    }

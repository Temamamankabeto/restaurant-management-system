...
export default function CreditAccountUsersPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Dialog open={cardsOpen} onOpenChange={setCardsOpen}><DialogContent className="max-h-[90vh] max-w-5xl overflow-y-auto print:max-h-none print:max-w-none print:overflow-visible print:border-0 print:shadow-none"><DialogHeader className="print:hidden"><DialogTitle>Generated credit smart cards</DialogTitle></DialogHeader><div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between print:hidden"><p className="text-sm text-muted-foreground">{cardUsers.length} card(s) ready for printing.</p><Button onClick={() => window.print()}><Printer className="mr-2 h-4 w-4" /> Print cards</Button></div><div className="grid gap-4 md:grid-cols-2 print:grid-cols-2">{cardUsers.map((user) => <CreditPass key={user.id} account={account} user={user} />)}</div></DialogContent></Dialog>
    </div>
  );
}

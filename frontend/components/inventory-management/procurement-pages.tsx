...
import { AlertTriangle, CheckCircle2, ClipboardList, PackageCheck, Plus, RefreshCcw, Search, Send, Truck, Users } from "lucide-react";
...
function extractError(error: unknown, fallback: string) { return error instanceof Error ? error.message : fallback; }
function canCreateSupplier() { return can(purchasePermissions.suppliersCreate); }
function canCreatePurchaseOrder() { return can(purchasePermissions.ordersCreate) || can(purchasePermissions.requestsCreate); }
function canSubmitPurchaseOrder() { return can(purchasePermissions.ordersSubmit); }
function canApprovePurchaseOrder() { return can(purchasePermissions.ordersApprove) || can(purchasePermissions.requestsApprove); }
function canReceivePurchaseOrder() { return can(purchasePermissions.ordersReceive) || can(inventoryPermissions.receive); }
...
function PageHeader({ title, description, icon: Icon }: { title: string; description: string; icon: typeof Truck }) {
  return <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><div className="flex items-center gap-2"><div className="rounded-xl bg-primary/10 p-2 text-primary"><Icon className="h-5 w-5" /></div><h1 className="text-2xl font-bold tracking-tight">{title}</h1></div><p className="mt-2 text-sm text-muted-foreground">{description}</p></div><Badge variant="secondary" className="w-fit">Purchaser → F&B validates → Manager approves → Stock Keeper receives</Badge></div>;
}
function poItemName(item: PurchaseOrderItemRow) { const inv = item.inventory_item ?? item.inventoryItem; return inv?.sku ? `${inv.name} (${inv.sku})` : inv?.name ?? `Item #${item.inventory_item_id}`; }
function poRemaining(item: PurchaseOrderItemRow) { return Math.max(Number(item.quantity ?? 0) - Number(item.received_quantity ?? 0), 0); }
function lineUnit(line?: PurchaseOrderItemRow) { const inv = line?.inventory_item ?? line?.inventoryItem; return inv?.base_unit ?? line?.unit ?? "pc"; }

function PoLinesTable({ lines }: { lines: PurchaseOrderItemRow[] }) {
  return <Table><TableHeader><TableRow><TableHead>Item</TableHead><TableHead>Ordered</TableHead><TableHead>Received</TableHead><TableHead>Remaining</TableHead><TableHead>Cost</TableHead></TableRow></TableHeader><TableBody>{lines.map((line) => <TableRow key={line.id}><TableCell>{poItemName(line)}</TableCell><TableCell>{formatBaseQuantity(line.quantity, lineUnit(line))}</TableCell><TableCell>{formatBaseQuantity(line.received_quantity ?? 0, lineUnit(line))}</TableCell><TableCell>{formatBaseQuantity(poRemaining(line), lineUnit(line))}</TableCell><TableCell>{formatMoney(line.unit_cost)} ETB</TableCell></TableRow>)}</TableBody></Table>;
}
...
function SupplierForm({ onDone }: { onDone?: () => void }) {
  ...
  function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const form = new FormData(event.currentTarget); mutation.mutate({ name: String(form.get("name") ?? ""), phone: String(form.get("phone") ?? ""), email: String(form.get("email") ?? ""), address: String(form.get("address") ?? ""), tax_id: String(form.get("tax_id") ?? ""), is_active: true }); }
...
export function PurchaseRequestsPage() {
  const [search, setSearch] = useState(""); const [status, setStatus] = useState("all"); const [open, setOpen] = useState(false);
...
function PurchaseOrderForm({ onDone }: { onDone?: () => void }) {
  ...
  const [supplierId, setSupplierId] = useState(""); const [status, setStatus] = useState<"draft" | "submitted">("submitted");
  ...
  function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const cleanLines = lines.filter((line) => line.inventory_item_id && Number(line.quantity) > 0).map((line) => { const inv = items.find((item) => String(item.id) === line.inventory_item_id); return { inventory_item_id: Number(line.inventory_item_id), quantity: Number(line.quantity), base_unit: inv?.base_unit ?? inv?.unit ?? "pc", unit_cost: Number(line.unit_cost || inv?.average_purchase_price || 0) }; }); if (!supplierId || !cleanLines.length) { toast.error("Select supplier and at least one item"); return; } mutation.mutate({ supplier_id: Number(supplierId), status, items: cleanLines }); }
...
function ReceiveOrderForm({ po, onDone }: { po: PurchaseOrderRow; onDone?: () => void }) {
  ...
  const mutation = useMutation({ mutationFn: (payload: any) => procurementService.receivePurchaseOrder(po.id, payload, "stock-keeper"), onSuccess: () => { toast.success("Ordered items received into inventory"); qc.invalidateQueries({ queryKey: ["procurement"] }); qc.invalidateQueries({ queryKey: ["inventory"] }); onDone?.(); }, onError: (error) => toast.error(extractError(error, "Failed to receive ordered items")) });
  function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const form = new FormData(event.currentTarget); const items = lines.filter((line) => Number(line.quantity) > 0).map((line) => { const poLine = (po.items ?? []).find((item) => item.id === line.purchase_order_item_id); return { purchase_order_item_id: line.purchase_order_item_id, quantity: Number(line.quantity), unit: lineUnit(poLine), expiry_date: line.expiry_date || undefined, batch_note: line.batch_note || undefined }; }); if (!items.length) { toast.error("Enter at least one received quantity"); return; } mutation.mutate({ note: String(form.get("note") ?? ""), items }); }
  return <form onSubmit={submit} className="space-y-4"><PoLinesTable lines={po.items ?? []} /><div className="space-y-2"><Label>Receive quantities</Label>{lines.map((line, index) => { const poLine = (po.items ?? []).find((item) => item.id === line.purchase_order_item_id); return <div key={line.purchase_order_item_id} className="grid gap-2 rounded-xl border p-3 md:grid-cols-[1fr_140px_160px_1fr]"><div><p className="font-medium">{poLine ? poItemName(poLine) : `Line #${line.purchase_order_item_id}`}</p><p className="text-xs text-muted-foreground">Remaining {poLine ? formatBaseQuantity(poRemaining(poLine), lineUnit(poLine)) : "—"}</p></div><Input type="number" step="0.001" max={poLine ? poRemaining(poLine) : undefined} value={line.quantity} onChange={(event) => setLines((old) => old.map((row, i) => i === index ? { ...row, quantity: event.target.value } : row))} /><Input type="date" value={line.expiry_date} onChange={(event) => setLines((old) => old.map((row, i) => i === index ? { ...row, expiry_date: event.target.value } : row))} /><Input placeholder="Batch note" value={line.batch_note} onChange={(event) => setLines((old) => old.map((row, i) => i === index ? { ...row, batch_note: event.target.value } : row))} /></div>; })}</div><div><Label>Receiving note</Label><Textarea name="note" /></div><Button disabled={mutation.isPending} type="submit"><RefreshCcw className="mr-2 h-4 w-4" />Receive into stock</Button></form>;
}
